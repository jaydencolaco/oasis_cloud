...
import { Camera, ChevronLeft, Loader2, Trash } from "lucide-react";
...
const EditProducts = ({ id }: { id: string }) => {
  ...
  useEffect(() => {
    const fetchDocs = async () => {
      if (!id) return;
      try {
        const userRef = doc(db, "products", id as string);
  ...
  const addEntry = () => {
    if (!detType || !detValue) {
      alert("Type and Value are required fields to add a new column");
      return;
    }
  ...
  const editProd = async (e: any) => {
    ...
    if (id) {
      try {
        ...
        setIsLoading(false);
        toast("Successfully Edited Product", {
  ...
  const handleDel = (index: number) => {
    console.log(subImg);
    setSubImg((prevFiles) => {
      const newFiles = prevFiles.filter((_, i) => i !== index); // Remove the file
        ...
        <div>
          <div className="md:w-[80%] w-[90%] py-10 mx-auto">
            <div className="flex gap-3">
              ...
              <h2 className="font-bold my-auto text-xl">Edit Product</h2>
            </div>
            {name ? (
